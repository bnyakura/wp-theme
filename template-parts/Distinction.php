<?php
/**
 * Distinction section — three (or more) info cards.
 *
 * Content comes from Dashboard → Distinction Cards (see inc/distinction-cards.php),
 * falling back to built-in defaults when none exist. Cards can open the enlist
 * modal (data-modal-open — handled by assets/js/modal.js), link to a page, or
 * open Google Maps directions.
 *
 * @package IGA
 */

$cards = iga_get_distinction_cards();
...
>
	<div class="mx-auto grid w-full max-w-[1400px] grid-cols-1 items-stretch gap-8 min-[1081px]:grid-cols-3">
		...
		<?php foreach ( $cards as $card ) : ?>
			...
			// Site-relative paths ("/training") resolve to home_url; full URLs pass through.
			$cta_url = ( ! empty( $card['cta_href'] ) && 0 === strpos( $card['cta_href'], '/' ) )
				? home_url( $card['cta_href'] )
				: $card['cta_href'];
			?>

			<article class="flex flex-col overflow-hidden rounded-[18px] border <?php echo esc_attr( $article_classes ); ?>">

				<!-- Media: image, or map when no image -->
				<div class="relative h-[200px] w-full shrink-0 overflow-hidden">
					...
					<?php else : ?>
						<iframe
							src="<?php echo esc_url( iga_map_embed_url() ); ?>"
							class="block h-[200px] w-full border-0"
							loading="lazy"
							referrerpolicy="no-referrer-when-downgrade"
							title="<?php esc_attr_e( 'Iron Gorilla Army location', 'iga' ); ?>"
						></iframe>
					<?php endif; ?>
				</div>
				...
				<div class="flex flex-1 flex-col px-6 pb-7 pt-6">
					...
					<?php if ( empty( $card['image'] ) && ! empty( $card['tag'] ) ) : ?>
					...
					<?php if ( ! empty( $card['cta_label'] ) ) : ?>
						<?php $cta_icon = ! empty( $card['cta_icon'] ) ? '<i class="fa-solid ' . esc_attr( $card['cta_icon'] ) . ' shrink-0 text-[0.9em] leading-none" aria-hidden="true"></i>' : ''; ?>

						<?php if ( 'directions' === $card['cta_action'] ) : ?>
							<a href="<?php echo esc_url( iga_directions_url() ); ?>" target="_blank" rel="noopener noreferrer" class="<?php echo esc_attr( $cta_classes ); ?>">
								<?php echo $cta_icon; // Already escaped above. ?>
								<?php echo esc_html( $card['cta_label'] ); ?>
							</a>
						<?php elseif ( 'link' === $card['cta_action'] && $cta_url ) : ?>
							<a href="<?php echo esc_url( $cta_url ); ?>" class="<?php echo esc_attr( $cta_classes ); ?>">
								<?php echo $cta_icon; // Already escaped above. ?>
								<?php echo esc_html( $card['cta_label'] ); ?>
							</a>
						<?php else : ?>
							<button type="button" data-modal-open="<?php echo esc_attr( $card['cta_action'] ); ?>" class="<?php echo esc_attr( $cta_classes ); ?>">
								<?php echo $cta_icon; // Already escaped above. ?>
								<?php echo esc_html( $card['cta_label'] ); ?>
							</button>
						<?php endif; ?>
