...
| **Background Overlay Color** | Color picker (with opacity) | No | Only shown once a Background Image is set. Wash colour + opacity placed over the photo — click the swatch and use the picker's own opacity slider. Default: cream at 80% opacity, matching the original design. Set the opacity all the way down for the photo untouched, or up to ~100% to hide it almost entirely behind a solid colour. |
