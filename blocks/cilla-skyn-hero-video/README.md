...
| **Poster / Fallback Image** | Image | No | Shown while the video loads (`<video poster>`), and rendered as a static background instead if no video is set at all. Falls back to a placeholder image when this is also empty. |
...
- **Background is video, not image**: no Laptop/Tablet image tiers — just one Desktop video and an optional Mobile video, switched via a `<source media>` breakpoint inside a single `<video>` element (autoplay, muted, loop, `playsinline`). If no video is set at all, the Poster / Fallback Image renders as a plain `<img>` instead, so the block never has to render an empty background.
