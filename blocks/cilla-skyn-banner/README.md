...
| **Image Position** | Select | No | Which part of the image stays in view once Image Fit crops it (CSS `object-position`) — Center (default), or nudged to Top/Bottom/Left/Right or a corner. Use this to keep the subject in frame when Image Fit is Cover and the photo gets cropped tighter than expected on some screens. Leave empty for Center. |
...
- **Image Fit** and **Image Position** each map directly to a Tailwind `object-*` utility class on the `<img>` — `render.php` uses a literal lookup array for each (e.g. `cover`/`contain`/`fill`/`none`/`scale-down` → their matching class names) rather than building the class name from the field's value at runtime, since Tailwind's build only generates CSS for class names it can actually find as literal text in the theme's files (see `src/input.css`'s top comment) — a dynamically-concatenated class name would be invisible to it and silently produce no styling. Image Position only has a visible effect once Image Fit actually crops something (Cover, or Contain/Scale Down when the image and its box don't share the exact same aspect ratio).
