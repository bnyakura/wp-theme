...
- **Image Fit** maps directly to a Tailwind `object-*` utility class on the `<img>` — `render.php` uses a literal lookup array (`cover`/`contain`/`fill`/`none`/`scale-down` → their matching class names) rather than building the class name from the field's value at runtime, since Tailwind's build only generates CSS for class names it can actually find as literal text in the theme's files (see `src/input.css`'s top comment) — a dynamically-concatenated class name would be invisible to it and silently produce no styling.
