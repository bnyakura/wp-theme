...
- **Mobile layout (below 768px) is deliberately taller than the content needs** (`min-h-[1320px]` on the text/image grid vs. `min-h-[560px]` at `min-[768px]:` and up), confirmed by real screenshots at 360/375/390/430/767px, not guessed. The Mobile background photo is a portrait shot with a person's face roughly in its lower half (`object-cover` + `object-top`, so the crop always anchors to the *top* of the source photo, never the default centre) — with the section only as tall as the text content (the original behaviour), `object-fit: cover` on a portrait image inside a comparatively short, wide box has very little vertical crop headroom, so the face ends up rendered directly behind the eyebrow/heading/description text with no room to separate the two. Making the mobile section substantially taller pushes the `cover`-scaled image further down as a whole (the text block itself stays pinned near the top via `justify-start`, unlike `min-[768px]:justify-center` at desktop widths), so the face consistently lands below all of the text instead of behind it. Tuned specifically against the narrowest common width (360px, where the two buttons wrap to two rows and the footer note wraps to two lines, both of which eat into the same vertical budget) — if a future Mobile photo has the face positioned even lower in the frame, this value only needs to go up, never down.
